...
def ExtractDataForKVAE(config, path_to_GSs, path_to_GSs_cells, path_to_images_folder,
                       path_to_inputs_to_final_model, path_to_inputs_to_final_model_full, check_mode, use_IMU,
                       path_to_acc = None, path_to_orient = None):
    ...
    if (path_to_acc is not None) and (path_to_orient is not None) and use_IMU:
        ...
        if (not os.path.exists(path_to_GSs) # GS folder not exists?
            ...
            or not os.path.exists(path_to_orient) # angular velocity folder not exists?
            or not os.listdir(path_to_images_folder)): # image subfolders don't exists?
            print ('Data in {}, {}, {} or {} does not exist'.format(path_to_images_folder, path_to_GSs, path_to_acc, path_to_orient))
    ...
    orient     = np.zeros((numberOfSequences, sequenceLength, 4))
    ...
    for idx in range(numberOfPoints):
        ...
        if (path_to_acc is not None) and (path_to_orient is not None) and use_IMU:
            ...
            orient[sequence_number, currBatch_idx, :]     = orientation[idx, :]
    ...
    if (path_to_acc is not None) and (path_to_orient is not None) and use_IMU:
        final_data = {'images':data, 'odometry': odom ,'controls':controls, 'params' : params,
                      'acceleration': acc, 'orientation': orient, 
